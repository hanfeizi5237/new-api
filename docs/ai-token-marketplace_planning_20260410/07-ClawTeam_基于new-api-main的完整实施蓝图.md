...
- `M1` 是否完成，以 `18-MVP-Lite与需求规格分层基线.md` 为准
- 原始完整需求继续保留为 `Pilot MVP / Marketplace V1` 的目标输入
- 若“完整需求说明”和“当前阶段任务书”存在冲突，优先按分层基线解释为阶段差异，而不是当前实现遗漏
...
16. 敏感操作必须满足最小权限、二次验证与审计要求，不能只靠后台菜单隐藏控制。
17. 卖家收入确认点不能前移到支付成功，必须遵循“履约确认 / 到期释放 / 冲销补偿”的结算口径。
...
- `usage_ledger / settlement_entry` 的事件口径、尾差收口、到期释放规则以后续 `16-结算、佣金、退款与冲销公式设计.md` 为准
- 管理端敏感动作与权限边界以后续 `17-RBAC、敏感操作与审计权限矩阵设计.md` 为准
- 异步任务、重试、死信和补偿口径以后续 `19-异步事件、任务重试与补偿机制设计.md` 为准
- DTO、分页、错误码与 webhook 返回契约以后续 `20-API契约与错误码规范.md` 为准
...
- 资金来源优先级与 token quota 行为以 `15-Token与Entitlement绑定及资金源优先级设计.md` 为准
- 卖家收入确认点与 `settlement_entry` 写入口径以 `16-结算、佣金、退款与冲销公式设计.md` 为准
- usage finalize 修复、lot 到期释放和其他异步补偿语义以 `19-异步事件、任务重试与补偿机制设计.md` 为准
- 管理端、买方端和支付 webhook 的 DTO/错误码约束以 `20-API契约与错误码规范.md` 为准
...
- 本阶段不以 `settlement release / dispute close / withdrawal` 为完成条件
- 上述能力归属以 `18-MVP-Lite与需求规格分层基线.md` 的阶段映射为准
...
- 上述缺口应按 `18-MVP-Lite与需求规格分层基线.md` 解释为后续阶段能力
- 不应把 `M2 / Pilot MVP` 能力直接回算为 `M1` 未完成
