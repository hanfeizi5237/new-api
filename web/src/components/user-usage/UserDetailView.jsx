      ...
      {summary && (
        <>
          ...
          <div className='mb-6 grid grid-cols-2 lg:grid-cols-5 gap-3'>
            ...
            <MetricCard label='Token 用量' value={renderNumber(summary.total_tokens || 0)} />
...
const ModelDistributionTab = ({ model_distribution = [] }) => {
  ...
  const columns = [
    ...
    { title: '输入 Token', dataIndex: 'prompt_tokens', render: (v) => renderNumber(v || 0) },
    { title: '输出 Token', dataIndex: 'completion_tokens', render: (v) => renderNumber(v || 0) },
...
const TimeTrendTab = ({ time_distribution = [] }) => {
  const columns = [
    ...
    { title: '调用次数', dataIndex: 'count', render: (v) => renderNumber(v || 0) },
    { title: '消耗额度', dataIndex: 'quota', render: (v) => renderQuota(v || 0, 2) },
    { title: 'Token 用量', dataIndex: 'tokens', render: (v) => renderNumber(v || 0) },
